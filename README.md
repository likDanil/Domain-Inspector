...
- **Формат вывода**:
  ...
  - **AdGuard** — через запятые (через запятые без пробела).
  - **3x-ui** — через запятые (через запятые с пробелом).
- **Фильтр доменов**:
  - **Все домены** — все домены
  - **Домены с ответами** — домены от которых был получен ответ
  - **Домены без ответов** — домены от которых не был получен ответ
...
- Выбери **формат вывода** (KeenDNS/AdGuard/3x-ui) — результат обновляется **мгновенно**.
...
- ✅ **Форматы вывода**: KeenDNS (строки), AdGuard (через запятые без пробела) и 3x-ui (через запятые с пробелом).
...
- ✅ **Роутеры Keenetic/Netcraze** (в настройках): можно добавлять несколько роутеров и управлять списком.  
